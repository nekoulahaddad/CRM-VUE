...
app.use("/passports", isAuth, express.static(path.join(__dirname + "/static/passports")));
...
events.on('call', async(e) => {
    try {
        if (e.call_state === 'Connected') {
            ...
            let user = await User.findOne({ 'inner_number': e.from.extension }).select('_id')
            if (user && user._id) {
                call.user = user._id
            } else {
...
events.on('recording', async(e) => {
    ...
    if (success) {
        let call = await Calls.findOneAndUpdate({ call_id: e.call_id }, {
            recording_id: e.recording_id,
