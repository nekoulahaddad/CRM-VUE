            ...
            regions: {
              $filter: {
                ...
                cond: {
                  $and: [
                    {
                      $eq: ['$$el.region', mongoose.Types.ObjectId(region.toString())],
                    },
                    {
                      $or: [
                        {
...
const createNewProduct = async (params, images, contentPath) => {
  ...
  product.slug = translit(_.kebabCase(product.title)).replace(/'/g, '').toLowerCase();
  product.meta = {
    title: product.title,
    ...
    for (let i = 0; i < images.length; i++) {
      if (contentPath) {
        ...
        await uploadFilesFromTempToFolder(
          ...
          UPLOADS_PATH,
          images[i].filename,
          `/catalog/${region}/categories/${product.category_id}/${product._id.toString()}`
        );
        product.images.push(images[i].filename);
      }
    }
...
exports.editProduct = async (req, res, next) => {
  try {
    ...
    // console.log("🚀 ~ file: products.js ~ line 541 ~ exports.editProduct=async ~ req.body", req.body)
    ...
    if (title !== undefined && title) {
      ...
      if (product.slug !== oldSlug && product.title.toLowerCase() !== oldTitle) changeTitle = true;
    }
    ...
    console.log('///////////////////////');
    try {
      feedsController.updateFeed(region, product.category_id);
    } catch (error) {
      console.log(error);
    }
    res.status(201).json({
