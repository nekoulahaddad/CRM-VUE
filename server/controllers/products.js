			...
			for (let i = 0; i < images.length; i++) {
				if (contentPath) {
					...
					await uploadFilesFromTempToFolder(
						...
						UPLOADS_PATH,
						images[i].filename,
						`/catalog/${region}/categories/${product.category_id
...
exports.editProduct = async (req, res, next) => {
	try {
		...
		if (images && images.length > 0) {
			...
			await removeUserDir(
				UPLOADS_PATH,
				`/catalog/${region}/categories/${product.category_id.toString()}/${product._id
				}`
			);
			await makeUserDir(
				UPLOADS_PATH,
...
exports.editProductRecommended = async (req, res, next) => {
	console.log("///////////////////////");
	console.log("Update list recommend Product");
	console.log("///////////////////////");
	try {
		const { productId, recomendedList, region } = req.body
		if (!productId && !recomendedList) return res.status(400).json({
			message: "NOT FOUND",
			desc: 'field PRODUCT_ID and RECOMENDED_LIST is reqiured'
		})

		const resUpdate = await Products.updateMany(
			{
				regions: {
					$elemMatch: {
						"product._id": mongoose.Types.ObjectId(productId),
			...
			{
				$set: {
					"regions.$.product.recomended": recomendedList,
				},
			}
		);
...
exports.editProductBuyed = async (req, res, next) => {
	console.log("///////////////////////");
	console.log("Update list buyed Product");
	console.log("///////////////////////");
	try {
		const { productId, buyedList, region } = req.body
		if (!productId && !buyedList) return res.status(400).json({
			message: "NOT FOUND",
			desc: 'field PRODUCT_ID and BUYED_LIST is reqiured'
		})
		...
		const resUpdate = await Products.updateMany(
			...
			{
				$set: {
					"regions.$.product.buyed": buyedList, 
				},
			}
		);
