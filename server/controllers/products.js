...
const fs = require('fs')
const { PRODUCTS_PATH, TEMP_PATH, UPLOADS_PATH } = require("../utils/path");
                                        ...
                                        },
                                        {
                                            $or: [{
                                                $regexFind: {
                                                    ...
                                                    regex: new RegExp('(\w|\s){0,}' + _.escapeRegExp(search) + '(\w|\s){0,}', 'ui')
                                                }
                                            },
                                            {
                                                $eq: [
                                                    "$$el.product.article", +search
                                                ]
                                            }]
                                        }
                                    ],
...
const createNewProduct = async (params, images, contentPath) =>{
        ...
        if (recomendsProductsIds) {
            if (
                Array.isArray(recomendsProductsIds) ||
                !recomendsProductsIds.length
            ) {
                let products_ids = [];
        ...
        if (!matchedProducts.length) {
            ...
            await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${product.category_id.toString()}/${product._id.toString()}`);
            
            for (let i = 0; i < images.length; i++) {
                if (contentPath){
                    await copyFilesFromTempToFolder(tempPath, UPLOADS_PATH, images[i].filename, `/catalog/${region}/categories/${product.category_id.toString()}/${product._id.toString()}`)
                    product.images.push(images[i].filename);
                } else {
                    await uploadFilesFromTempToFolder(tempPath, UPLOADS_PATH, images[i].filename, `/catalog/${region}/categories/${product.category_id}/${product._id.toString()}`)
                    product.images.push(images[i].filename);
                }
            ...
            for (let i = 0; i < images.length; i++) {
                if (contentPath){
                    ...
                    await uploadFilesFromTempToFolder(tempPath, UPLOADS_PATH, images[i].filename, `/catalog/${region}/categories/${product.category_id.toString()}/${product._id.toString()}`)
                    product.images.push(images[i].filename);
                }
            }
            product.path = `/uploads/catalog/${region}/categories/${product.category_id}/${product._id}/`;
            ...
            product._id = mongoose.Types.ObjectId(
                matchedProducts[0].results[0]._id
            );
            console.log(product);
