...
exports.getUsersBySearch = async(req, res, next) => {
    try {
        ...
        const usersBySearch = await User.find({
            ...
            .select(["_id", "surname", "name", "lastname", "phone", "position", "avatar"])
            .limit(4)
            .lean()
        console.log("///////////////////////");
        console.log("Users By search");
...
exports.addUser = async(req, res, next) => {
    try {
        ...
        let usersRegistered = await User.find({ phone: dataUser.phone }).countDocuments()
        ...
        if (newUser.role === 'manager') {
            let lastNumber = await User.find().select('number').sort({ number: -1 }).limit(1)
...
exports.resetPassword = async(req, res, next) => {
    try {
        ...
        let user = await User.findOne({ $and: [{'deleted': false}, {$or: [{ 'phone': login }, { 'login': login }] }]}).lean()
        console.log(user)
...
exports.editUser = async(req, res, next) => {
    try {
        ...
        const avatar = req.files["avatar"] && req.files['avatar'][0] ? req.files['avatar'][0] : req.files["avatar"]
...
exports.deleteUser = async(req, res, next) => {
    try {
        const userId =  Array.isArray(req.body.userId) ? req.body.userId : [ req.body.userId ]
        for (id of userId){
...
exports.resetInnerNumber = async(req, res, next) => {
    try {
        ...
        if (inner_number)
            user.inner_number = inner_number
...
exports.getUsersTreeList = async(req, res, next) => {
    try {
        ...
        if (options && options.department !== "all" && options.department !== null) {
            myMatch["department.value"] = {
                $in: [options.department],
            };
        }
        ...
        if (req.body.usersList && req.body.usersList !== null) {
            let userIds = req.body.usersList.map(item => mongoose.Types.ObjectId(item._id))
            myMatch["_id"] = {
                $nin: userIds,
            };
        }
