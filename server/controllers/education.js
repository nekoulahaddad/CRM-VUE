...
exports.getEducations = async (req, res, next) => {
    try {
        ...
        if (req.userRole !== 'superadmin') {
            ...
            //     if (data.department && data.department !== 'all' && data.role !== 'director') {
            //         myMatch = {
            //             ...myMatch,
            //             $or: [
            //                 {
...
exports.uploadDocument = async (req, res, next) => {
    try {
        ...
        const currentEducation = await Educations.findById(dataDocument.educationId)
        for (doc of req.files.document){
            await makeUserDir(EDUCATION_PATH, `/${currentEducation._id}/`)
            await uploadFilesFromTempToFolder(TEMP_PATH, EDUCATION_PATH, doc.filename, `/${currentEducation._id}/`)
...
exports.deleteEducation = async (req, res, next) => {
    try {
        ...
        const deletedEducation = await Educations.findOneAndDelete({_id: educationId})
        console.log("///////////////////////")
