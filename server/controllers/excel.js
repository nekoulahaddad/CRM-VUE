...
const {REGIONS_PATH, USERS_PATH, SEO_PATH, ONEC_PATH, FEEDS_PATH} = require('../utils/path')
    ...
    try {
        ...
        if (deletedUsers.length) {
            ...
            for (let i = 0; i < deletedUsers.length; i++) {
                ...
                ws.cell(i + 2, 1) // ФИО
                    .string(`${user.surname} ${user.name}${user.lastname ? ' ' + user.lastname : ''}`)
                    .style(globalStyle)
...
exports.getExcelFromUsersBirthday = async (req, res, next) => {
    try {
        ...
        if (acceptedUsers.length) {
            ...
            res.status(400).json({message: 'Дни рождения за текущий месяц не найдены'})
            return
        }
...
const getSubCategoriesIds = async (  region, categoryIds ) => { // доработать
    ...
    subCategoriesId = subCategoriesId[0] ? subCategoriesId[0].category_ids ? subCategoriesId[0].category_ids : [] : []
...
exports.exportCategoryProducts = async (req, res, next) => {
    try {
        ...
        productsFromRegion.forEach(product => {
            ...
            product.lastModified = moment(product.lastModified).format('DD.MM.YYYY hh:mm:ss')
        })
        worksheet.columns = [
        ...
        worksheet.eachRow({includeEmpty: false}, function (row, rowNumber) {
            ...
            switch(visible){
                case 'видимый' : worksheet.getCell(`B${rowNumber}`).font = {
                    color : { argb: '00FF00' }
                }
                break;
                case 'скрытый' : worksheet.getCell(`B${rowNumber}`).font = {
                    color : { argb: 'FF0000' }
                }
                break;
            }
        ...
        await workbook.xlsx.writeFile(path.join(SEO_PATH, `Товары_по_${categoryName}.xlsx`))
            .then( () =>{
...
exports.exportOrders = async (req, res, next) =>{
    try {
        const {region, client, status, executor, dates, startDate, endDate } = req.body
        const myMatch = {}
        ...
        ordersFromRegion.forEach((value) => {
            ...
            // })
            let productsArr = value.products.filter( p => p.title != undefined && p.category != undefined)
            ...
            let manager = value.manager.length ? `${value.manager[0].name} ${value.manager[0].surname} ${value.manager[0].lastname}` : ""
            let client = value.client.length ? `${value.client[0].name} ${value.client[0].lastname}` : ""
            worksheet.addRow({
                number: value.number,
                client: client,
...
function getUpdatedProduct(params, region){
    let set = {
        ...
        "regions.$[regionEl].product.full_club_cost": (params.club_cost * params.coef) || null,
    ...
    await worksheet.eachRow({includeEmpty: false}, (row, rowNumber) => {
        if(rowNumber != 1){
            ...
            switch(product.visible){
                case "видимый" : product.visible = true
                break;
                case "скрытый" : product.visible = false
                break;
                default : product.visible = false
            }
            ...
            values.length ? options.values = values.split(', ') : options.values = []
            product.prodOptions = []

            // product.path = path.join(__dirname, '..', worksheet.getCell((`K${rowNumber}`)).value) 
            product.path = path.join("/var/content",  worksheet.getCell((`K${rowNumber}`)).value || "")
            product.imagesStr = worksheet.getCell((`L${rowNumber}`)).value || ""

            product.imagesStr.length ? options.images = product.imagesStr.split(', ') : options.images = []
            product.images = []
            ...
            if (options.keys.length && options.values.length && options.keys.length == options.values.length){
...
exports.exportOneC = async (req, res, next) => {
    try {
        ...
        await workbook.xlsx.writeFile(path.join(ONEC_PATH, `/${regionName}.xlsx`))
            .then( () => {
...
exports.getGoodsFromRegion = async (req, res, next) => {
    try {
        ...
        await workbook.xlsx.writeFile(path.join(SEO_PATH, `/Товары_по_${regionTitle}.xlsx`))
...
async function generateExcel(req, res, region, categories) {
    try {
        ...
        for (let i = 0; i < categories.length; i++) {
            ...
            products = products[0] && products[0].products ? products[0].products : ''
            if (products && products.length) {
                ...
                for (let i = 0; i < products.length; i++) {
                    ...
                    if (product.title !== null && product.title !== 'null' && product.title !== undefined) {
                    ...
                    if (product.article !== null && product.article !== 'null' && product.article !== undefined) {
                    ...
                    if (product.cost !== null && product.cost !== 'null' && product.cost !== undefined) {
                        ws.cell(i + 2, 3) // Цена
                            .number(product.cost)
                            .style(globalStyle)
                    } else {
                    ...
                    if (product.club_cost !== null && product.club_cost !== 'null' && product.club_cost !== undefined) {
                    ...
                    for (let i = 0; i < product.options.length; i++) {
                        for (let key in product.options[i]) {
                            if (key !== "Артикул") options += `${[key]}: ${product.options[i][key]};\r\n`
                        }
                    }
                    console.log(options)
                    ...
                    ws.cell(i + 2, 7) // Кол-во в упаковке
                        .number(product.coef)
                        .style(globalStyle)
                    if (product.UUID !== null && product.UUID !== 'null' && product.UUID !== undefined) {
                        ws.cell(i + 2, 8) // UUID
                            .string(product.UUID)
                            .style(globalStyle)
                    } else {
...
exports.getOrdersFromRegion = async (req, res, next) => {
    try {
        ...
        ordersFromRegion.forEach((value) => {
            worksheet.addRow({
                ...value,
                delivery: (value.delivery && typeof(value.delivery.city) !== 'undefined') ? `${value.delivery.city}, ул. ${value.delivery.street}, д. ${value.delivery.house}, ${value.delivery.building}, кв. ${value.delivery.appt}` : value.delivery,
                status: value.status.title,
                typeDelivery: value.typeDelivery === 'transport' ? 'Доставка транспортом' : value.typeDelivery === 'courier' ? 'Доставка курьером' : value.typeDelivery === 'pickup' ? 'Самовывоз' : '',
                payment: value.payment === 'cash' ? 'Наличными' : value.payment === 'bankTransfer' ? 'Банковский перевод' : value.payment === 'cartSite' ? 'Картой онлайн' : value.payment === 'cartReceiving' ? 'Картой при получении' : '',
        ...
        await workbook.xlsx.writeFile(path.join(SEO_PATH, `/Заказы_${regionTitle.split(' ').join('_')}.xlsx`))
        ...
        for (let i = 0; i < global.users.length; i++) {
            ...
            if (user.userId === req.userId) {
                ...
                setTimeout(() => {
                    global.io.to(req.userId).emit("DownloadExcel", {
                        link: `/uploads/seo/Заказы_${regionTitle.split(' ').join('_')}.xlsx`,
...
exports.getCategoryFeedYML = async (req, res, next) => {
    try {
        ...
        for (let p of productsFromRegion){
            ...
            let ya_category_id = categories.find( c => c._id.toString() === category_id).id
            let data = {
                '@id': p.article,
                name: p.title,
                vendor: vendor,
...
exports.getExcelFromClients = async (req, res, next) => {
    const page = +req.query.page - 1
        ...
        for (c of result){
            let fullname = (c.name ? c.name + ' ' : '') + (c.lastname ? c.lastname : '')
            let data = {
                fullname: fullname,
                email: c.email,
                phone: c.phone,
        ...
        await workbook.xlsx.writeFile(path.join(SEO_PATH, filename))
        .then( () => {
...
exports.getExcelFromTasks = async (req, res, next) => {
    const page = +req.query.page - 1
        ...
        const user = await Users.findOne({_id: mongoose.Types.ObjectId('5fb7ae283a8ca00699551693')})
        ...
        for (t of tasks){
            let executor = (t.executor.name ? t.executor.name + ' ' : '') + (t.executor.surname ? t.executor.surname : '')
            let data = {
                initiator: initiator,
                executor: executor,
                title: t.title,
...
exports.getExcelManagerCodes = async (req, res, next) => {
        ...
        for (t of users){
            let fio = (t.name ? t.name + ' ' : '') + (t.lastname ? t.lastname + ' ' : '') + (t.surname ? t.surname : '')
            let data = {
                fio: fio,
                code: t.number
