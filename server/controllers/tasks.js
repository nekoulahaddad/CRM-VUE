    ...
    if (options.dates !== null || options.dates !== "all") {
      ...
      switch (options.dates) {
        ...
        case "month":
          left = new Date(today);
          right = new Date(today);
          (y = left.getFullYear()), (m = left.getMonth());
        ...
        case "year":
          left = new Date(today);
          right = new Date(today);
          (y = left.getFullYear()), (left = new Date(y, 0, 1));
  ...
  try {
    ...
    if (dataTask.department) {
      ...
      users = await User.find({
        department: dataTask.department,
        _id: { $nin: [executors[0]._id, req.userId] },
      })
        .select("_id")
        .lean();
    } else {
    ...
    if (!!addedTask.parent_id || addedTask.executors.length === 1) {
      let user = await User.findById(addedTask.executors[0]._id).select(
        "_id name surname lastname"
      );
...
exports.editTask = async (req, res, next) => {
  try {
    ...
    if (!!task.parent_id) {
      ...
      let user = await User.findById(task.executors[0]._id).select(
        "_id name surname lastname"
      );
      console.log(user);
      task.executor = user;
...
exports.editTaskStatus = async (req, res, next) => {
  try {
    ...
    const updateTask = await currentTask.updateOne({
      status: status._id,
...
exports.deleteTask = async (req, res, next) => {
  try {
    const taskId = Array.isArray(req.body.taskId)
      ? req.body.taskId
      : [req.body.taskId];
    for (t of taskId) {
      ...
      if (!deletedTask.parent_id) {
        let deletedSubTasks = await Tasks.find({ parent_id: deletedTask._id })
          .select("_id")
          .lean();
...
exports.getExcelFromTasks = async (req, res, next) => {
  try {
    ...
    if (tasks && tasks.length) {
      ...
      ws.cell(1, 1).string("Название").style(globalStyle);
      ws.cell(1, 2).string("Описание").style(globalStyle);
      ws.cell(1, 3).string("Автор").style(globalStyle);
      ws.cell(1, 4).string("Исполнитель").style(globalStyle);
      ws.cell(1, 5).string("Дата создания").style(globalStyle);
      ws.cell(1, 6).string("Дедлайн").style(globalStyle);
      ws.cell(1, 7).string("Статус").style(globalStyle);
      ws.cell(1, 8).string("Комментарий").style(globalStyle);
      for (let i = 0; i < tasks.length; i++) {
        ...
        if (
          task.initiator !== null &&
          task.initiator !== "null" &&
          task.initiator !== undefined
        ) {
          ws.cell(i + 2, 3)
            .string(
              task.initiator.surname +
                " " +
                task.initiator.name +
                " " +
