...
exports.getCategories = async (req, res, next) => {
    try {
        ...
        const googleDoc = await gDocProducts.findOne({region: mongoose.Types.ObjectId(region)})
            .populate({
            path: 'region',
            model: 'Regions',
            select: "_id title value",
        })
        let categoriesList = categories && categories[0] ? categories[0].categories : []
...
exports.getCategoriesViews = async (req, res, next) => {
    ...
    if (nesting >= 1){
        ...
        res.status(200).json({
            ...
            filters: filtersAggregate && filtersAggregate[0] ? filtersAggregate[0].filters : [],
    ...
    try {
        ...
        if (typeof (views) !== 'undefined') {
            if (views && views.length) {
                ...
                let viewsList = JSON.parse(views).map((view) => mongoose.Types.ObjectId(view._id.toString()))
        ...
        // Делаем cSlug
        category.cSlug = translit(_.kebabCase(categoryName)).replace(/'/g, '').toLowerCase()
        ...
        if (!matchedCategories.length) {
            ...
            if (images) {
                if (images.categoryImage) {
                    // Image
                    category.img = ''
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}/`)
                    await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryImage[0].filename, `/catalog/${region}/categories/${category._id}/`)
                ...
                if (images.categorySlide) {
                    ...
                    category.slide = ''
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}/`)
                    await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categorySlide[0].filename, `/catalog/${region}/categories/${category._id}/`)
                ...
                if (images.categoryBanner) {
                    ...
                    category.banner = ''
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}/`)
                    await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryBanner[0].filename, `/catalog/${region}/categories/${category._id}/`)
                ...
                if (images.categoryIcon) {
                    ...
                    category.icon = ''
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}/`)
                    await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryIcon[0].filename, `/catalog/${region}/categories/${category._id}/`)
            ...
            if (images) {
                if (images.categoryImage) {
                    ...
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id.toString()}`)
                    await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryImage[0].filename, `/catalog/${region}/categories/${category._id}/`)
                    category.img = images.categoryImage[0].filename
                    category.path = `/uploads/catalog/${region}/categories/${category._id}/`
                } else {
...
exports.editCategory = async (req, res, next) => {
    try {
        ...
        if (typeof (categoryName) !== 'undefined' && categoryName !== category.categoryName) {
        ...
        if (Array.isArray(images.categoryImage)) {
            // Image
            await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}`)
            await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryImage[0].filename, `catalog/${region}/categories/${category._id.toString()}/`)
            category.img = images.categoryImage[0].filename
            category.path = `/uploads/catalog/${region}/categories/${category._id}/`
        }
        ...
        if (Array.isArray(images.categorySlide)) {
            // Image
            await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}`)
            await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categorySlide[0].filename, `catalog/${region}/categories/${category._id.toString()}/`)
            category.slide = images.categorySlide[0].filename
            category.slidePath = `/uploads/catalog/${region}/categories/${category._id}/`
        ...
        if (Array.isArray(images.categoryBanner)) {
            // Image
            await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}`)
            await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryBanner[0].filename, `catalog/${region}/categories/${category._id.toString()}/`)
            category.banner = images.categoryBanner[0].filename
            category.bannerPath = `/uploads/catalog/${region}/categories/${category._id}/`
        }
        if (Array.isArray(images.categoryBannerMob)) {
            // Image
            await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}`)
            await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryBannerMob[0].filename, `catalog/${region}/categories/${category._id.toString()}/`)
        ...
        if (Array.isArray(images.categoryIcon)) {
            // Icon
            await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${category._id}`)
            await uploadFilesFromTempToFolder(TEMP_PATH, UPLOADS_PATH, images.categoryIcon[0].filename, `catalog/${region}/categories/${category._id.toString()}/`)
            category.icon = images.categoryIcon[0].filename
            category.iconPath = `/uploads/catalog/${region}/categories/${category._id}/`
        }
        ...
        { arrayFilters: [{ "regionEl.region" : mongoose.Types.ObjectId(region)}] }).lean()
        res.status(201).json({
            message: "EDITED",
            category: category
        })
...
exports.copyCategory = async (req, res, next) => {
    ...
    function cloneAndChangeCategory(oldCategory, targetRegion){
        ...
        if (oldCategory.meta && oldCategory.meta.title !== undefined ) oldCategory.meta.title = ''
        if (oldCategory.meta && oldCategory.meta.keywords !== undefined ) oldCategory.meta.keywords = ''
        if (oldCategory.meta && oldCategory.meta.description !== undefined) oldCategory.meta.description = ''   

        if ( oldCategory.path !== '/uploads/') oldCategory.path = newPath
        if ( oldCategory.slidePath !== '/uploads/') oldCategory.slidePath = newPath
        if ( oldCategory.bannerPath !== '/uploads/') oldCategory.bannerPath = newPath
    ...
    function cloneAndChangeProduct(oldProduct, targetRegion){
        if (oldProduct.meta && oldProduct.meta.title !== undefined) oldProduct.meta.title = ''
        if (oldProduct.meta && oldProduct.meta.keywords !== undefined) oldProduct.meta.title = ''
        if (oldProduct.meta && oldProduct.meta.description !== undefined ) oldProduct.meta.title = ''  

        if ( oldProduct.path !== '/uploads/') oldProduct.path = `/uploads/catalog/${targetRegion}/categories/${oldProduct.category_id}/${oldProduct._id}/`
    ...
    try {
                    ...
                    $filter: {
                        ...
                        cond: {
                            $and: [{
                                $eq: [
                                    '$$el.region',
                                    mongoose.Types.ObjectId(targetRegion)
                                ]
                            }, {
                                $eq: [
                                    '$$el.category._id',
        ...
        if (targetCluster[0] && targetCluster[0].category && !targetCluster[0].category.deleted) {
        ...
        if (ids.length) {
            if (ids[0].categoriesIds) categoriesIds = categoriesIds.concat(ids[0].categoriesIds)
                ...
                results: {
                    $filter: {
                        ...
                        cond: {
                            $and: [{
                                $eq: [
                                    '$$el.region', mongoose.Types.ObjectId(region)
                                ]
                            }, {
                                $eq: [
                                    '$$el.category.deleted', false
                                ]
                            }, {
                                $in: [
                                    '$$el.category.parent_id', categoriesIds
                                ]
                            }, {
                                $eq: [
                                    '$$el.category.nesting', 2
                                ]
                            }]
                        }
                    },
        ...
        if (thirdids.length && thirdids[0].categoriesIds) {
            ...
            if (thirdids[0].categoriesIds) categoriesIds = categoriesIds.concat(thirdids[0].categoriesIds)
            if (thirdids[0].categories) categories = categories.concat(thirdids[0].categories)
        }
                ...
                $project: {
                    results: {
                        $filter: {
                          input: '$regions',
                          as: 'region',
                          cond: { $eq: ['$$region.region', mongoose.Types.ObjectId(region)] }
                        }
                    }
                },
            },
        ...
        if (targetCluster[0] && targetCluster[0].category && targetCluster[0].category.deleted){
            ...
            for (let i = 0; i < categories.length; i++){   
                ...
                if (!updatedCategory) {
                    const createdCategory = cloneAndChangeCategory(categories[i], targetRegion)
                    ...
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${categories[i]._id}/`)
                    await makeUserDir(UPLOADS_PATH, `/catalog/${targetRegion}/categories/${categories[i]._id}/`)
                    
                    await copyFolderRecursively(
                        path.join(UPLOADS_PATH, `/catalog/${region}/categories/${categories[i]._id}/`),
                        path.join(UPLOADS_PATH, `/catalog/${targetRegion}/categories/${categories[i]._id}/`)
                    )
            ...
            for (let i = 0; i < products.length; i++){
                ...
                if (!updatedCategory) {         
                    ...
                    await makeUserDir(UPLOADS_PATH, `/catalog/${region}/categories/${products[i].category_id}/${products[i]._id}`)
                    await makeUserDir(UPLOADS_PATH, `/catalog/${targetRegion}/categories/${products[i].category_id}/${products[i]._id}`)

                    await copyFolderRecursively(
                        path.join(UPLOADS_PATH, `/catalog/${region}/categories/${products[i].category_id}/`),
                        path.join(UPLOADS_PATH, `/catalog/${targetRegion}/categories/${products[i].category_id}/`)
                    )  
            ...
            for (let i = 0; i < groups.length; i++) {             
                ...
                if (!createdGroupResult.nModified) console.log(`Group ${groups[i]._id} not created`)
            }
        ...
        // Если в целевом регионе категории нет
            ...
            for (let i = 0; i < categories.length; i++) {
                ...
                if (!createdCategoryResult.nModified) console.log(`Category ${categories[i]._id} not created`)
            }
            ...
            for (let i = 0; i < products.length; i++) {             
                ...
                if (!createdProductResult.nModified) console.log(`Product ${products[i]._id} not created`)
            }
            for (let i = 0; i < groups.length; i++) {             
...
const getProductsIds = async (  region, categoryIds ) => {
    ...
    return {
        prodIds : products[0] ? products[0].products_ids ? products[0].products_ids : [] : [],
        groupIds: products[0] ? products[0].group_ids ? products[0].group_ids : [] : []
    }
}
...
const setCategoryVisibility = async (catIds, region, visible, userId ) => {
    await Categories.updateMany({
        ...
        $addToSet: {
            "regions.$[regionEl].category.updates": {user: userId, type: "set_visibility"}
        }
    },{
        arrayFilters: [{"regionEl.region" : mongoose.Types.ObjectId(region)} ]
    }
...
const performVisibilityUpdate = async (catIds, region, visible, userId) => {
    ...
    catIds = catIds.map( id => { return mongoose.Types.ObjectId(id)})
